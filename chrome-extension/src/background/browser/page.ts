  ...
  async inputTextElementNode(useVision: boolean, elementNode: DOMElementNode, text: string): Promise<void> {
    ...
    try {
      ...
      const currentValue = await element.evaluate(el => {
        ...
        return ''; // Return empty string for other element types
      });

      // Check if the field is already filled. If so, skip filling unless the new text is different.
      if (currentValue.trim() !== '' && currentValue.trim() === text.trim()) {
        logger.debug(`Field for element ${elementNode} is already filled with the same text. Skipping.`);
        return; // Skip filling if already filled with the same text
      }
