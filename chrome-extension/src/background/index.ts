        ...
        switch (message.type) {
          ...
          case 'new_task': {
            ...
            if (message.files && Array.isArray(message.files) && message.files.length > 0) {
              logger.info('Received file attachments:', (message.files as any[]).map((f: any) => ({ name: f.name, type: f.type, size: f.size })));
              
              // Process files and extract text for LLM context
              const fileHandler = new FileHandler();
              try {
                const extractedTexts = await fileHandler.processFiles(message.files);
                const fileContents = extractedTexts.map((text: string, index: number) => {
                  const file = message.files[index];
                  return `File: ${file.name}\nType: ${file.type}\nContent:\n${text}\n---\n`;
                }).join('\n');

                // Add file contents to the task context
                message.task = `Task: ${message.task}\n\nAttached Files:\n${fileContents}`;
              } catch (error) {
                logger.error('Error processing files:', error);
                port.postMessage({ type: 'error', error: 'Failed to process attached files' });
                return;
              }
            }
          ...
          case 'follow_up_task': {
            ...
            if (message.files && Array.isArray(message.files) && message.files.length > 0) {
              ...
              try {
                ...
                // Add file contents to the follow-up task context
                message.task = `Task: ${message.task}\n\nAttached Files:\n${fileContents}`;
              } catch (error) {
                logger.error('Error processing files:', error);
                port.postMessage({ type: 'error', error: 'Failed to process attached files' });
