...
export default class Page {
  ...
  isFileUploader(elementNode: DOMElementNode, maxDepth = 3, currentDepth = 0): boolean {
    ...
    if (tagName === 'input' && elementNode.attributes.type === 'file') {
      return true;
    }

    if (elementNode.parent) {
      return this.isFileUploader(elementNode.parent, maxDepth, currentDepth + 1);
    }

    return false;
  }

  async _checkAndHandleNavigation(): Promise<void> {
    if (!this._puppeteerPage) {
      return;
    }
    ...
    try {
      ...
      await this._puppeteerPage.waitForNavigation({ waitUntil: 'networkidle0', timeout: 5000 });
      const newUrl = this._puppeteerPage.url();

      if (currentUrl !== newUrl) {
        logger.info('Navigation detected:', { currentUrl, newUrl });
        // Update the state after navigation
        this._state.url = newUrl;
        this._cachedState = null; // Invalidate cached state
      }
    } catch (error) {
      logger.warning('Navigation check failed:', error);
    }
  }

  async waitForPageAndFramesLoad(timeout = 30000): Promise<void> {
    if (!this._puppeteerPage) {
      throw new Error('Puppeteer is not connected');
    }

    try {
      await this._puppeteerPage.waitForTimeout(500); // Initial wait

      await this._puppeteerPage.waitForFunction(
        () => {
          const readyState = document.readyState;
          const iframeReady = Array.from(document.querySelectorAll('iframe')).every(iframe => {
            try {
          ...
          return readyState === 'complete' && iframeReady;
        },
        { timeout },
      );
    } catch (error) {
      logger.warning('waitForPageAndFramesLoad timed out or failed:', error);
    }
  }
}
