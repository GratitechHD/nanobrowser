...
const SidePanel = () => {
  ...
  // Update handleSendMessage to accept files
  const handleSendMessage = async (text: string, files?: File[]) => {
    ...
    try {
              ...
              new Promise<{ name: string; type: string; size: number; data: string }>((resolve, reject) => {
                ...
                reader.onload = () => {
                  resolve({
                    ...
                    type: file.type,
                    size: file.size,
                    data: (reader.result as string).split(',')[1], // Remove data:...;base64,
                  });
                };
      ...
      if (hasFiles) {
        filesPayload = await filesToPayload(files!);
      }
      ...
      if (isFollowUpMode) {
        ...
        console.log('follow_up_task sent', trimmedText, tabId, sessionIdRef.current, filesPayload);
      } else {
        // Send as new task
        await sendMessage({
          type: 'new_task',
        ...
        console.log('new_task sent', trimmedText, tabId, sessionIdRef.current, filesPayload);
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : String(err);
  ...
  useEffect(() => {
    const loadFavorites = async () => {
      try {
        const prompts = await favoritesStorage.getAllPrompts();
        setFavoritePrompts(prompts);
        console.log('Favorite prompts loaded', prompts);
      } catch (error) {
